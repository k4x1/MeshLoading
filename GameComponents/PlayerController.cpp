...
void PlayerController::Start() {
    ...
    void PlayerController::Update(float dt) {
        ...
        glm::vec3 dir(0.0f);
        if (inp.GetKey(GLFW_KEY_W)) { dir.z -= 1.0f; Debug::Log("W pressed"); }
        if (inp.GetKey(GLFW_KEY_S)) { dir.z += 1.0f; Debug::Log("S pressed"); }
        if (inp.GetKey(GLFW_KEY_A)) { dir.x -= 1.0f; Debug::Log("A pressed"); }
        if (inp.GetKey(GLFW_KEY_D)) { dir.x += 1.0f; Debug::Log("D pressed"); }
        if (inp.GetKey(GLFW_KEY_Q)) { dir.y += 1.0f; Debug::Log("Q pressed"); }
        if (inp.GetKey(GLFW_KEY_E)) { dir.y -= 1.0f; Debug::Log("E pressed"); }
